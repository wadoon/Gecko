 ...
 * color of a displayed [Region].
 */
class InspectorColorPicker(actionManager: ActionManager, Region: Region) : ColorPicker(),
    InspectorElement<ColorPicker> {
    init {
        value = Region.color
        Region.colorProperty.addListener { _: ObservableValue<out Color?>?, _: Color?, newValue: Color? ->
            value = newValue
        }
        ...
        onAction = EventHandler {
            actionManager.run(
                actionManager.actionFactory
                    .createChangeColorRegion(Region, value)
            )
        }
    }
