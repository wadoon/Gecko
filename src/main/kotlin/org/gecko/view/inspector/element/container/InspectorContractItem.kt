 ...
 * region-specific-[Inspector][org.gecko.view.inspector.Inspector]. The [InspectorContractItem]
 * contains thus [InspectorPreconditionField]s, [InspectorPostconditionField]s and an {link
 * InspectorInvariantField} in the case of a region.
 */
class InspectorContractItem : VBox, InspectorElement<VBox> {
    ...
    constructor(actionManager: ActionManager, state: State?, Contract: Contract) {
        ...
        val preConditionField: InspectorAreaField =
            InspectorPreconditionField(actionManager, Contract)
        contractFields.add(preConditionField)
        preConditionField
            .prefWidthProperty()
            .bind(widthProperty().subtract(InspectorElement.Companion.FIELD_OFFSET))
        contractConditions.add(preConditionLabel, 0, 0)
        contractConditions.add(preConditionField, 1, 0)

        val postConditionLabel = InspectorLabel(ResourceHandler.Companion.post_condition)
        val postConditionField: InspectorAreaField =
            InspectorPostconditionField(actionManager, Contract)
        ...
        // Contract name
        contractNameBox.children.addAll(
            InspectorCollapseContractButton(contractFields),
            InspectorRenameField(actionManager, Contract),
            deleteButtonSpacer,
    ...
    constructor(actionManager: ActionManager, Region: Region) {
        ...
        addContractItem(
            ...
            InspectorPreconditionField(actionManager, Region.contract),
            0,
            regionConditions
        )
        addContractItem(
            ResourceHandler.Companion.post_condition,
            InspectorPostconditionField(actionManager, Region.contract),
            1,
            regionConditions
        )
        addContractItem(
            ResourceHandler.Companion.invariant,
            InspectorInvariantField(actionManager, Region),
            2,
            regionConditions
        )
