 ...
 * [position][Point2D] and the current [EditorViewModel] for setting the correct position for the
 * created port.
 */
class CreateVariableAction internal constructor(val gModel: GModel, val position: Point2D) :
    Action() {
    private lateinit var createdPort: Port

    @Throws(GeckoException::class)
    override fun run(): Boolean {
        createdPort = gModel.viewModelFactory.createPort(gModel.currentEditor!!.currentSystem)
