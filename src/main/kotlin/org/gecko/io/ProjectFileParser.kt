...
inline fun <reified T : Any> decodeFromStream(it: InputStream) =
    gson.fromJson(InputStreamReader(it), T::class.java)
...
class ProjectFileParser : FileParser {
    ...
    override fun parse(file: File): GModel {
        val wrapper =
            file.inputStream().use { JsonParser.parseReader(InputStreamReader(it)) }.asJsonObject
